...
void loop() {
  ...
  if (sensor.new_data) {
    ...
    if (data >= 1.0) cout = cout + data + "F";
    else if (data >= 1e-3) cout = cout + data * 1e3 + "mF";
    else if (data >= 1e-6) cout = cout + data * 1e6 + "uF";
    else cout = cout + data * 1e9 + "nF";
    cout += '\t';
    cout += sensor.v_max;
    cout += 'V';
    cout += '\t';
...
void Capacitance_Sensor::charge(void) {
  ...
  // What happens if charged
    ...
    // Measurement successful
      ...
      Capacitance_Sensor::farads = time_charge / (Capacitance_Sensor::r_val[Capacitance_Sensor::selector] + 0.037);
...
void Capacitance_Sensor::stop_all(void) {
  ...
  for (int i = 0; i < Capacitance_Sensor::total_charge_pins; i++) {
    digitalWrite(Capacitance_Sensor::pin_charge[i], LOW);
  }
