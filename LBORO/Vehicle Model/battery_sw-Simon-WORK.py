...
class Battery_Model(ElectricityClass.ElectricalDevice):
    ...
    def __init__(self, kwargs, name='battery'):
        ...
        self._eta           = kwargs['batt_eta'] if 'batt_eta' in kwargs else 1.0
    ...
    def update(self, dt):
        ...
        if error is error_t.none:
            joules_transferred  = (2.0 - self._eta) * self.i * self.v * dt * 10.0 # Joule is a watt-second
            self._joules       -= joules_transferred
        elif error is error_t.e_full:
            if self.i >= 0.0:
                joules_transferred  = (2.0 - self._eta) * self.i * self.v * dt # Joule is a watt-second
                self._joules       -= joules_transferred
            else:
                self.i = 0.0
                self.p = 0.0 
